...
class MinecraftDrawing:
    ...
    # draw x-y letter
    def draw_letter(self, x1, y1, z1, radius, letter, alpha, beta, gamma, ratio, blockType, blockData=0):
        # create a x-y letter
        if isascii(letter):
            if len(letter) == 1:
                if isinstance(letter, int):
                    letter = str(letter)
                try:
                    letter = letter.decode("cp1252")
                except:
                    letter = letter.decode("iso8859_1")
                print letter
                print ord(letter)
                bitmap = FONTS['8x8'][ord(letter)][3]
                ratio1 = math.trunc(ratio)
                for i in range(8):
                    for j in range(8):
                        if 0 != bitmap[i] & (1 << (8-j)):
                            x = (4 - j)*ratio1
                            y = radius
                            z = (3 - i)*ratio1
                            for k in range(-math.trunc(ratio1/2),ratio1-math.trunc(ratio1/2)):
                                for l in range(-math.trunc(ratio1/2),ratio1-math.trunc(ratio1/2)):
                                    #x軸回転角alpha
                                    xx = x+k
                                    yx = (y * math.cos(math.radians(alpha)) - (z+l) * math.sin(math.radians(alpha)))
                                    zx = (y * math.sin(math.radians(alpha)) + (z+l) * math.cos(math.radians(alpha)))
                                    #y軸回転角beta
                                    ...
                                    yz = round(yz, 3)
                                    zz = round(zz, 3)
                                    self.drawPoint3d(x1 + xz, y1 + yz, z1 + zz, blockType, blockData)
            else:
    ...
    # draw x-y japanese
    def draw_japanese(self, x1, y1, z1, radius, letter, alpha, beta, gamma, ratio, blockType, blockData=0):
        # create a x-y japanese
        if is_japanese(letter.decode("utf-8")):
            #if isinstance(letter, int):
                #letter = str(letter)
            if len(letter.decode("utf-8")) == 1:
                #try:
                    #letter = letter.decode("cp1252")
                #except:
                    #letter = letter.decode("iso8859_1")
                print letter
                #print ord(letter)
                bitmap = FONTSJP['japanese'][letter][3]
                ratio1 = math.trunc(ratio)
                for i in range(8):
                    for j in range(8):
                        if 0 != bitmap[i] & (1 << (8-j)):
